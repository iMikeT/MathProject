...
plt.legend([r'$x = -3$',r'$y = 19$',r'$x = 3$',r'$y = 16$',r'$y = 17.4 - 0.5x$'], fontsize=12, loc='upper right')
...
def plot_MCMC_trace(xdata, ydata, trace, scatter=False, **kwargs):
    ...
    plt.title('Error Ellips of the Intercept and the Slope', fontsize=15)
...
def plot_MCMC_model(xdata, ydata, trace):
    ...
    plt.plot(xfit, mu, 'r-', linewidth=2)
    plt.fill_between(xfit, mu - sig, mu + sig, color='lightgray')
 
    plt.xlabel(r'Value for $x$', fontsize=12)
    plt.ylabel(r'Value for $y$', fontsize=12)
    plt.legend(['Data','Approximation'], fontsize=12, loc='upper right')
    plt.title('Approximation of Linear Solution', fontsize=15)
